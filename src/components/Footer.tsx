...
const socialLinks = [
  ...
  {
    name: 'Telegram',
    href: 'https://t.me/FlowFi_Official',
    icon: (
...
export default function Footer() {
  ...
  return (
    <footer className="relative bg-gray-900/95 backdrop-blur-md border-t border-gray-800">
      {/* Background Pattern */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 lg:gap-12">
          ...
          >
            ...
            <p className="font-body text-sm text-gray-400 leading-relaxed max-w-xs">
              Transforming how money moves around the world through decentralized PayFi protocol.
            </p>
          </motion.div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {menuItems.map((item, index) => (
                <motion.button
                  ...
                  className="text-gray-400 hover:text-white transition-colors duration-200 font-body text-sm text-left py-1 hover:translate-x-1 transform transition-transform cursor-pointer"
                  whileHover={{ x: 4 }}
            ...
            <div className="flex flex-wrap gap-3">
              {socialLinks.map((social, index) => (
                <motion.div
                  ...
                  whileHover={social.href ? { scale: 1.1, y: -2 } : {}}
                  whileTap={social.href ? { scale: 0.95 } : {}}
                  initial={{ opacity: 0, scale: 0.8 }}
                  whileInView={{ opacity: 1, scale: 1 }}
                  viewport={{ once: true }}
                  transition={{ duration: 0.3, delay: 0.3 + index * 0.1 }}
                  onClick={social.href ? () => window.open(social.href, '_blank') : undefined}
                  title={social.name}
                >
                  <div className={`${social.href ? 'text-gray-300 hover:text-white' : 'text-gray-500'} transition-colors duration-200`}>
                    {social.icon}
                  </div>
                </motion.div>
              ))}
        ...
        >
          <p className="font-body text-sm text-gray-400">
            © {currentYear} FlowFi. All rights reserved. 
      ...
      {/* Decorative Elements */}
      <motion.div 
        className="absolute bottom-0 left-0 w-96 h-96 bg-gradient-to-br from-purple-600/10 to-cyan-600/10 rounded-full blur-3xl pointer-events-none"
        animate={{ 
          scale: [1, 1.1, 1],
          opacity: [0.3, 0.5, 0.3]
        }}
